...
public:
    ...
    // Eg: If there are 4 variables, 2, that is 10 in binary is represented as
    // 0010
        ...
        for (int i = 0; i < max; i++) {
            for (int j = i; j < max; j++) {
                ...
                if (isGreyCode(minterms[i], minterms[j])) {
                    checked[i] = 1;
                    checked[j] = 1;
                    if (!in_vector(newminterms, replace_complements(
                                                    minterms[i], minterms[j])))
                        newminterms.push_back(
...
int main() {
    ...
    while (check == 'y') {
        ...
        string temp = "";
        cout << "Enter the minterms(RANGE=0-" << pow(2, no) - 1
             << ") separated by comma:" << endl;
        cin >> temp;
